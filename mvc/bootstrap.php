...
class Bootstrap extends \stdClass
{
    ...
    {
        if (!isset($this->_pathRoot)) die('You must run setPathRoot($path)');
    ...
    public function setPathRoot($path)
    {
        ...
        $this->_pathConfig = $this->_pathRoot . 'var/';
    }
    ...
    private function _initController()
    {
        ...
        if (file_exists($this->_pathController . $module . "controller/" . strtolower($this->_uriController) . '.php')) {
            ...
            if (isset($this->_uriMethod)) {
                ...
                else $this->controller->{$this->_uriMethod}();
            } else {
    ...
    {
        if (!file_exists($this->_pathConfig . 'config.php')) {
            die(__CLASS__ . ": error (missing config)\n
                You must create your base config model here: {$this->_pathConfig}config.php\n
                <pre>
                &lt;?php\n
                class Model {}
                </pre>
