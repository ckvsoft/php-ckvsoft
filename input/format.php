...
class Format {
    ...
    public function ifgt($str, $param) {
        ...
        if (is_int($str)) {
            if ($str > $param[0]) return $param[1];
        }
        if (is_string($str)) {
            if (strlen($str) > $param[0]) return $param[1];
        }
    }
    ...
    public function hash($str, $param) {
        ...
        if (count($param) == 1) $ctx = hash_init($param[0]);
        else $ctx = hash_init($param[0], HASH_HMAC, $param[1]);
